...
router.get('/shopmanage', function(req, res, next) {
  ...
  if(loginbean.role==4){
    ...
    sequelize.query(sql,{replacements: [loginbean.id]}).then(function(shopRs){
        ...
        pageSize=2;;    //每页显示条目数
        startPoint = (page-1)*pageSize; //查询起点位置
        sizeCount=0;  //总记录数
        sumPage=0;
          ...
          sequelize.query(sqlCount,{replacements:[loginbean.id],type:sequelize.QueryTypes.QUERY}).then(function(rs){  
             rsjson = JSON.parse(JSON.stringify(rs[0]));
            sizeCount=rsjson[0].count;
            sumPage=Math.ceil(sizeCount/pageSize);//Math.floor,Math.round  向上取整
          sql ='select * from goods where uid=? limit ?,?';
          sequelize.query(sql,{replacements: [loginbean.id,startPoint,pageSize],type: sequelize.QueryTypes.QUERY}).then(function(goodsRs){
          res.render('home/shopmanage', {shoptypeRs:shoptypeRs[0],shopRs:shopRs[0],rs:goodsRs[0],tagflag:req.query.tagflag});
             });
          });  
         });
        });        
...
router.post('/upgoods', function(req, res, next) {
  ...
  var form = new formidable.IncomingForm();   //创建上传表单 
    ...
    form.parse(req, function (err, fields, files) { 
       ...
       loginbean = req.session.loginbean;
       fields.uid = loginbean.id; 
        console.log(fields.goodsid);

        console.log(files.goodsimg.name);

        if(files.goodsimg.name){
       fields.goodsimg=files.goodsimg.path.replace('public','');
        updsql='update goods set goodsname=?,price=?,goodsimg=?,goodsintro=?,typeid=? where id=?';  
        sequelize.query(updsql,{replacements:[fields.goodsname,fields.price,fields.goodsimg,fields.editorValue,fields.typeid,fields.goodsid],type:sequelize.QueryTypes.UPDATE}).then(function(rs){  
       ...
       res.redirect('./shopmanage?tagflag=1');   
          ...
          fields.goodsimg=fields.oldGoodsImg;
           updsql='update goods set goodsname=?,price=?,goodsintro=?,typeid=? where id=?'; 
          sequelize.query(updsql,{replacements:[fields.goodsname,fields.price,fields.editorValue,fields.typeid,fields.goodsid],type:sequelize.QueryTypes.UPDATE}).then(function(rs){  

            console.log(12212121212121212);
             res.send('修改成功');     
       res.redirect('./shopmanage?tagflag=1');  
             });
        }   
